...
class UserHandler {
    ...
    async getById(req, res) {
        try {
            ...
            let userResult;
            if (isNaN(Number(id))) {
                // Если это не число, ищем по username
                userResult = await User.getByLogin(id);
            } else {
                // Иначе ищем по числовому ID
    ...
    async create(req, res) {
        try {
            const { username, email, password_hash, role_name } = req.body;
    ...
    async update(req, res) {
        try {
            ...
            const updatedUser = await User.update(id, { username, email, password_hash, role_id });
